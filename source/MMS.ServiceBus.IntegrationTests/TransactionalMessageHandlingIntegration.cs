namespace MMS.ServiceBus
{
    ...
    [TestFixture]
    public class TransactionalMessageHandlingIntegration
    {
        ...
        public async Task WhenOneMessageSent_WithinATransactionWhichIsRolledback_DoesNotInvokeHandlers()
        {
            ...
            this.context.AsyncHandlerCalls.Should().NotBeInvoked();
            this.context.HandlerCalls.Should().NotBeInvoked();
        }

        [Test]
        public async Task WhenOneMessageSent_WithinATransactionWhichIsCommitted_InvokesSynchronousAndAsynchronousHandlers()
        {
            using (var tx = new TransactionScope(TransactionScopeOption.Required, TransactionScopeAsyncFlowOption.Enabled))
            {
                await this.sender.Send(new Message { Bar = 42 });
            ...
            await this.context.Wait(asyncHandlerCalls: 1, handlersCalls: 1, replyHandlerCalls: 1);

            this.context.AsyncHandlerCalls.Should().BeInvokedOnce();
            this.context.HandlerCalls.Should().BeInvokedOnce();
            this.context.ReplyHandlerCalls.Should().BeInvokedOnce();
        }
        ...
        public async Task Foo()
        {
            using (var tx = new TransactionScope(TransactionScopeOption.Required, TransactionScopeAsyncFlowOption.Enabled))
            {
                await this.sender.Send(new Message { Bar = 42, Throw = false });

                tx.Complete();
            }

            await this.context.Wait(asyncHandlerCalls: 1, handlersCalls: 0, replyHandlerCalls: 0);
            await Task.Delay(50000);

            this.context.AsyncHandlerCalls.Should().BeInvokedOnce();
            this.context.HandlerCalls.Should().NotBeInvoked();
            this.context.ReplyHandlerCalls.Should().NotBeInvoked();
        }
